...
class MainMenuState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.ACCEPT) {
			#if mobile
			lime.system.System.showKeyboard();
			...
			openfl.Lib.current.stage.addEventListener(openfl.events.TextEvent.TEXT_INPUT, onMobileInput);
			#end
		}
		...
		if (controls.BACK) {
			#if mobile
			lime.system.System.hideKeyboard();
			if (openfl.Lib.current.stage.hasEventListener(openfl.events.TextEvent.TEXT_INPUT))
				openfl.Lib.current.stage.removeEventListener(openfl.events.TextEvent.TEXT_INPUT, onMobileInput);
			#end
	...
	#if mobile
	private function onMobileInput(e:openfl.events.TextEvent):Void {
		if (e.text != null && e.text.length > 0) {
			processCheatString(e.text.toUpperCase());
		}
	}
	#end

	private function processCheatString(letter:String):Void {
		penkaruTimeout = 0;
		penkaru += letter;
		
		if (openfl.Assets.exists('assets/sounds/tts/${letter.toLowerCase()}.ogg')) 
			FlxG.sound.play(flixel.system.FlxAssets.getSound('assets/sounds/tts/${letter.toLowerCase()}'));

		if (penkaru == 'FAT' && !fatMode) {
			#if mobile lime.system.System.hideKeyboard(); #end
